...
import {
  ...
  DollarSign, AlertCircle, ArrowRight, Clock, Calendar
} from "lucide-react";
import {
...
export default function DashboardContent() {
  ...
  // "now" is stable here — this component never runs on the server
  const now = new Date();
  ...
  const urgentDeliveries = useMemo(() =>
    deliveries.filter((d) => d.status !== "entregue" && d.status !== "cancelado").slice(0, 4),
  [deliveries]);
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-3">
            ...
            {activeProjects.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-4">Nenhum projeto em andamento</p>
            )}
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-5">
          ...
          <div className="space-y-3">
            ...
            {urgentDeliveries.length === 0 && (
              <p className="text-sm text-gray-400 text-center py-4">Nenhuma entrega pendente</p>
            )}
          </div>
        </div>
      </div>
