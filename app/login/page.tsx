...
export default function LoginPage() {
  ...
  useEffect(() => { if (mounted && currentUser) router.replace("/dashboard"); }, [mounted, currentUser, router]);

  // Verifica se é primeiro acesso via API
  useEffect(() => {
    if (!mounted) return;
    fetch("/api/users")
      .then((r) => r.json())
      .then((users: unknown[]) => {
        if (users.length === 0) router.replace("/cadastro");
        else setCheckingFirst(false);
      })
      .catch(() => setCheckingFirst(false));
  }, [mounted, router]);
  ...
  if (!mounted || checkingFirst || currentUser) {
    return (
